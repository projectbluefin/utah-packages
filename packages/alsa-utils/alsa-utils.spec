# Parity recipe (utah-packages#146): restores kernel mixer state across reboots
# (alsactl, alsa-restore.service/alsa-state.service, 90-alsa-restore.rules,
# /usr/share/alsa/init). Hummingbird ships alsa-lib and alsa-ucm only, so this
# is what Utah was missing for sound. Version tracks alsa-lib (built in
# lockstep upstream).
#
# Source0 below is an ftp:// URL, so tools/bootstrap_upstream_sources.py can
# never lock it (it requires a direct HTTP(S) payload). The source lock in
# config/upstream-sources.json is therefore written by hand, pointing at the
# Fedora pkgs mirror and keyed on the SHA-512 in this package's `sources` file
# -- the same pattern alsa-firmware uses. Do not "fix" it to the ftp URL.
...
%changelog
* Wed Jul 15 2026 Fedora Release Engineering <releng@fedoraproject.org> - 1.2.16-2
