# Parity recipe (utah-packages#149): ppp needs libpcap (Requires) and linux-atm
# (BuildRequires linux-atm-libs-devel; ppoatm links libatm.so.1), neither in
# Hummingbird nor the factory. ppp is staged one wave behind them: stage 1 here
# vs stage 0 for libpcap/linux-atm (config/upstream-sources.json) so its build
# root sees libatm.so.1. NetworkManager ships NetworkManager-ppp from its own
# repo, so ppoatm is otherwise uninstallable.
