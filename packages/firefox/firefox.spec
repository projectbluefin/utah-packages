...
#
# Turned off here, using the switch the recipe already provides for it. A PGO
# build is two compiles with an instrumented run between them, and the
# instrumented run needs a wayland compositor -- Koji gives that a builder for
# most of a day, while this factory has a four-core GitHub runner and a
# six-hour job limit. It also turns off the WASI SDK sub-build, which is
# another cmake and cargo toolchain compile before Firefox itself starts.
#
# What this costs: no profile-guided optimisation and no RLBox sandboxing of
# the media libraries, so the browser is slower than Fedora's and one
# defence-in-depth layer thinner. What it does not cost is correctness or
# completeness -- this is Fedora's own non-PGO configuration, which is why
# the release carries the .npgo tag the recipe appends for it.
#
# Firefox is here for one reason: anaconda-webui's browser dependency, which
# nothing else in a Hummingbird-only transaction can answer. Revisit if the
# factory ever gets a builder that can afford the full build.
...
if test -x /work/tools/sccache && test -r /work/tools/sccache.env; then
  ...
  # Hand the cache to the build system rather than wrapping CC: mach drives
  # the C, C++, rust and host compiles itself, and only --with-ccache reaches
  # all of them. The PGO path sets CCACHE_DISABLE, but PGO is off here.
  echo "ac_add_options --with-ccache=/work/tools/sccache" >> .mozconfig
fi
