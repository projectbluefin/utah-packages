...
Source11: fish-4.6.0-vendor.tar.gz
...
# Vendored Rust crates. Fedora 44 ships no binary crate RPMs, so the
# system-crate model below cannot satisfy fish's Rust dependencies; this
# tree is built entirely offline against it.
# ponytail: committed directly in packages/fish/ rather than listed in the
# `sources` lookaside manifest, because this factory cannot push to
# src.fedoraproject.org. source_pipeline only fetches sources-file entries,
# so the committed tarball is staged by build-stage's `cp -a` and never
# triggers a lookaside 404. Re-list in `sources` once lookaside access exists.
tar -xf %{SOURCE11}
...
rm -f fishshell-cargo-config.toml
