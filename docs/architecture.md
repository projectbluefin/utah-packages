...
asserts the package list is non-empty, which a list of 371 satisfies while
...
The root Packit configuration and the source lock both cover all 371 recipes:

| check | result |
| --- | ---: |
| `ls -d packages/*/ \| wc -l` | `371` |
| entries under `.packit.yaml:packages` | `371` |
| entries under `config/upstream-sources.json:packages` | `371` |

`python3 tools/validate.py` reports:

```text
validated 371 source RPMs
```
