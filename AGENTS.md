...
- Buildroots are declared by image digest in
  [`config/buildroot-lock.json`](config/buildroot-lock.json), which must name
  exactly the pin in [`config/buildroot-image`](config/buildroot-image); move
  both with `tools/buildroot_pin.py set`, never by hand. A rebuild snapshots
  the NEVRAs the pinned image carries and publishes a package/source/buildroot
  manifest beside the OCI digest. `just check` fails when the lock and the pin
  differ. See
