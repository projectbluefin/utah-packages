...
| `No match for argument: rust-*-devel` (F44 buildroot has no crate RPMs) | Fedora's `%cargo_generate_buildrequires` system-crate model | Rewrite spec to vendor crates (`cargo vendor` + `%cargo_prep -v vendor` + `bundled(crate(...))` provides) |
