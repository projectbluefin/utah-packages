...
is still part of the contract.  A third rule joins them: the runtime
contract's ``[unavailable]`` packages are policy exclusions, so they must leave
the contract instead of being reported as parity Hummingbird still owes.
...
class MainTests(unittest.TestCase):
    ...
    def test_malformed_unavailable_section_is_rejected(self):
        # package_section validates the shape, so a malformed [unavailable]
        # fails loudly instead of silently altering the contract.
...
class RealManifestTests(unittest.TestCase):
    ...
    def test_real_policy_removes_firefox_from_the_real_contract(self):
        # End-to-end pin for issue #112: the runtime contract marks firefox
        # unavailable (Utah ships the Flatpak), so the gap report must not
        # carry it as parity Hummingbird still owes.
        root = Path(__file__).resolve().parent.parent
