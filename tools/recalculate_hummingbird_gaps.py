...
The contract is every ``packages`` list in the Bluefin manifest except
``[excluded]``, minus the ``[unavailable]`` exclusions declared in the runtime
contract.  A package Utah has deliberately stopped shipping (each entry carries
an issue-backed reason) is not parity debt Hummingbird still owes, so it must
not appear in ``missing_from_hummingbird``.
"""
